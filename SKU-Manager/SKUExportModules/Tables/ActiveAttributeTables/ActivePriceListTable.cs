    ...
    class ActivePriceListTable : ExportTable
    {
        ...
        public override DataTable getTable()
        {
            ...
            foreach (string sku in skuList)
            {
                ...
                row[90] = msrp;                                                     // msrp
                double runCharge = Math.Round(msrp * 0.05) / 0.6 + Convert.ToInt32(list[51]) - 1;
                if (runCharge > 8)
                {
                    row[91] = 8;
                }
        ...
        protected override string[] getSKU()
        {
            ...
            SqlCommand command = new SqlCommand("SELECT top 15 SKU_Ashlin FROM master_SKU_Attributes WHERE Active = \'TRUE\' ORDER BY SKU_Ashlin", connection);
            connection.Open();
        ...
        private ArrayList getData(string sku)
        {
            ...
            adapter = new SqlDataAdapter("SELECT Colour_Description_Short, Colour_Description_Extended, Colour_Description_Short_FR, Colour_Description_Extended_FR FROM ref_Colours WHERE Colour_Code = \'" + color + "\';", connection);
            adapter.Fill(table);
            for (int i = 0; i <= 3; i++)
            {
                list.Add(table.Rows[0][i]);
